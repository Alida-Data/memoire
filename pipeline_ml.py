...
class ModelTrainer:
    ...
    def log_mlflow(self):
        ...
        pipeline_ml.set_experiment("Detection_Fraude_Financial")

        X_train, X_test, y_train, y_test = self.prepare_data()

        with pipeline_ml.start_run(run_name="Entrainement_XGBoost_Final_Cleaned"):
            # Logging des hyperparamètres
            pipeline_ml.log_params(self.params)
            ...
            pipeline_ml.log_metric("f1_score", f1)
            pipeline_ml.log_metric("accuracy", accuracy)
            pipeline_ml.set_tag("Statut", "Pipeline_Production_Valide")
            ...
            try:
                ...
                pipeline_ml.log_artifact("confusion_matrix.png")
                plt.close()
                ...
                pipeline_ml.log_artifact("roc_curve.png")
                plt.close()
...
if __name__ == "__main__":
    uvicorn.run("pipeline_ml:app", host="0.0.0.0", port=8000, reload=True)